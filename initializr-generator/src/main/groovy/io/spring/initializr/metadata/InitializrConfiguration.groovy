...
class InitializrConfiguration {
	...
	static class Env {
		...
		/**
		 * The "PwikiConfig" that are referenced in this instance, identified by an
		 * arbitrary identifier that can be used in deriving url and siteid.
		 */
		final Map<String, PiwikConfig> piwikConfigs = [:]
		...
		void merge(Env other) {
			...
			other.piwikConfigs.each { id, piwikConfig ->
				if (!piwikConfigs[id]) {
					piwikConfigs[id] = piwikConfig
				}
			}
