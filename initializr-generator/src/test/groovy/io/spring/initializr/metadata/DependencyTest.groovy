...
class DependencyTest {
	...
	void resolveInvalidMapping() {
		def dependency = new Dependency(id: 'web')
		dependency.version = '0.1.0.RELEASE'
		dependency.version << new Dependency.Mapping(
				versionRange: 'foo-bar', version: '0.1.0.RELEASE')
		thrown.expect(InvalidInitializrMetadataException)
		thrown.expectMessage('Invalid scope package must be one of [compile, runtime, compileOnly, provided, test')
		dependency.setScope('package')
		dependency.resolve()
	}

	@Test
	void resolveMatchingMapping() {
		def dependency = new Dependency(id: 'web', description: 'A web dependency', version: '0.2.0.RELEASE',
		...
		validateResolvedWebDependency(dependency.resolve(Version.parse('1.1.0.RELEASE')), '0.2.0.RELEASE')
		validateResolvedWebDependency(dependency.resolve(Version.parse('1.2.0.RELEASE')), '0.2.0.RELEASE')
		validateResolvedWebDependency(dependency.resolve(Version.parse('2.1.3.M1')), '0.2.0.RELEASE') // default
	}
