...
class ProjectGeneratorBuildTest extends AbstractProjectGeneratorTest {

	@Parameterized.Parameters(name = "{0}")
	static Object[] parameters() {
		Object[] maven = ["maven", "pom.xml"]
		Object[] gradle = ["gradle", "build.gradle"]
		Object[] parameters = [gradle]
		parameters
	}
    ...
    ProjectGeneratorBuildTest(String build, String fileName) {
		this.build = build
	...
	void versionOverride() {
		...
		if (request.javaVersion == "1.8" && request.build == "gradle") {
			project.sourceCodeAssert("$fileName")
					.equalsTo(new ClassPathResource("project/$build/1.8/version-override-$assertFileName"))
		} else{
			project.sourceCodeAssert("$fileName")
	...
	void bomWithVersionProperty() {
		...
		if (request.javaVersion == "1.8" && request.build == "gradle") {
			project.sourceCodeAssert("$fileName")
					.equalsTo(new ClassPathResource("project/$build/1.8/bom-property-$assertFileName"))
		} else {
			project.sourceCodeAssert("$fileName")
	...
	void compileOnlyDependency() {
		...
		if (request.javaVersion == "1.8" && request.build == "gradle") {
			project.sourceCodeAssert("$fileName")
					.equalsTo(new ClassPathResource("project/$build/1.8/compile-only-dependency-$assertFileName"))
		}
		else{
			project.sourceCodeAssert("$fileName")
