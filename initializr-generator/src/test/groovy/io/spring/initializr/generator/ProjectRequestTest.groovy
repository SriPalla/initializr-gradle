...
//	    assertEquals metadata.types.default.id, request.type
	...
	void resolve() {
		...
		assertEquals 'Build type not detected', 'gradle', request.build
	...
	void resolveDependencyVersion() {
		...
		request.style << 'org.foo:bar'
		request.resolve(metadata)
		assertDependency(request.resolvedDependencies[0], 'org.foo', 'bar', '1.2.0.RELEASE')
		...
		assertDependency(anotherRequest.resolvedDependencies[0], 'org.foo', 'bar', '1.2.0.RELEASE')
	}
