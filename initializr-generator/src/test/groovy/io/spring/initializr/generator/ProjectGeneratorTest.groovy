 ...
 * @Contributor plq6883
 */
class ProjectGeneratorTest extends AbstractProjectGeneratorTest {

    @Test
    @Ignore
    //No signature of method: io.spring.initializr.generator.ProjectGenerator.initializeModel()
    ...
    void defaultProject() {
        def request = createProjectRequest('web')
        request.build = 'gradle'
        generateProject(request).isJavaProject('com.usaa.template', 'TemplateCreationApplication').isGradleProject()
        verifyProjectSuccessfulEventFor(request)
    }
    ...
    void noDependencyAddsRootStarter() {
        ...
        request.build = 'gradle'
        generateProject(request).isJavaProject('com.usaa.template', 'TemplateCreationApplication').isGradleProject()//.pomAssert()
    ...
    @Test
    @Ignore
    // Currently project generator handles gradle projects
    ...
    @Test
    @Ignore
    // Currently project generator supports Gradle [jar, ear & zip]
    ...
    @Test
    @Ignore
    // Currently project generator is generating java projects
    ...
    void customBaseDirectory() {
        ...
        generateProject(request).hasBaseDir('my-project')
                .isJavaProject('com.usaa.template', 'TemplateCreationApplication')
                .isGradleProject()
    }
    ...
    @Test
    void groovyWithGradleUsesJavaDir() {
        def request = createProjectRequest('web')
        request.type = 'gradle-project'
        request.language = 'groovy'
        generateProject(request).isGradleProject().isJavaProject('com.usaa.template', 'TemplateCreationApplication')
    }
